...
TAPE_CASSETTE_TYPES = {
    "Normal (Fe)": {"hi": 0.0,  "lo": 0.5,  "headroom": 1.0, "desc": "Type I — tlenek żelaza, standard"},
    "CrO₂ (Chrome)": {"hi": 2.0,  "lo": 0.0,  "headroom": 1.15, "desc": "Type II — lepsze wysokie i dynamika"},
    "Metal": {"hi": 3.5,  "lo": -0.5, "headroom": 1.3, "desc": "Type IV — najszersze pasmo i headroom"},
}

# Dolby NR (symulacja charakteru odtwarzania): (hf_cut_db, mid, expand_feel)
TAPE_DOLBY = {
    "Off": {"hf": 0.0,  "mid": 0.0, "sat_mul": 1.0, "desc": "Bez redukcji szumu"},
    "Dolby B": {"hf": -1.5, "mid": 0.0, "sat_mul": 0.95, "desc": "~10 dB NR w wysokich (B)"},
    "Dolby C": {"hf": -3.0, "mid": -1.0, "sat_mul": 0.9, "desc": "~20 dB NR, szersze pasmo (C)"},
}
...
class SdrWindow(Adw.ApplicationWindow):
    ...
    def _map_tape(s):
        ...
        headroom = cass["headroom"]
        # Drive + typ kasety (Metal = wiecej headroomu = lagodniejsza saturacja)
        thr = 0.9 - (drive / 100.0 * 0.8) / headroom
        thr = clamp(thr, 0.05, 0.95)
        vol = 1.0 + (drive / 100.0 * 0.8)
        ratio = (1.0 + (comp / 100.0 * 9.0)) * dolby["sat_mul"]

        # Warmth + charakter kasety + Dolby (odtwarzanie)
        band0 = warmth / 100.0 * 4.0 + cass["lo"]
        band1 = dolby["mid"]
        band2 = -(warmth / 100.0 * 3.0) + cass["hi"] + dolby["hf"]
        if mpx:
            # filtr MPX: sciecie okolic pilotu 19 kHz / skrajnych wysokich
            band2 -= 2.5

        # Stereo / Mono / lekki "Pro Logic" (szerokosc sceny)
        if stereo_mode == "Mono":
            stereo_w = 0.0
        elif prologic:
            stereo_w = 0.55  # lekkie poszerzenie a'la matrix
        else:
            stereo_w = 1.0
