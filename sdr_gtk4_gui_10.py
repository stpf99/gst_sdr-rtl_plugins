...
FX_LED_LABELS = {
    ...
    "tape": "TAPE", "spatial": "SPAT", "phantom": "PHAN",
}


class AnalogFmScale(Gtk.DrawingArea):
    """Analogowa skala FM 88–108 MHz ze znacznikami presetów (klik = strojenie)."""
    ...
    def _on_draw(self, area, cr, width, height):
        # tło panelu — ciemny aluminium / bakelit
        cr.set_source_rgb(0.12, 0.13, 0.14)
        cr.paint()

        pad_l, pad_r, pad_t = 18, 18, 28
        scale_y = height * 0.55
        usable = width - pad_l - pad_r

        # płytka skali
        cr.set_source_rgb(0.18, 0.20, 0.18)
        cr.rectangle(pad_l - 6, pad_t - 8, usable + 12, height - pad_t - 8)
        cr.fill()
        ...
        # znaczniki presetów (zielone diody-kropki)
        for name, hz in self._markers:
            if not (FM_BAND_LO <= hz <= FM_BAND_HI):
                continue
            x = pad_l + self._hz_to_x(hz, usable)
            cr.set_source_rgb(0.35, 0.95, 0.45)
            cr.arc(x, scale_y + 12, 3.2, 0, 6.2832)
            cr.fill()

        # wskazówka (pomarańczowa)
        ...
        cr.set_font_size(16)
        txt = f"{self._freq / 1e6:.2f} MHz"
        cr.move_to(pad_l, height - 10)
        cr.show_text(txt)
...
class LedChip(Gtk.Button):
    """Dioda/przycisk FX — kolorowy segment, klik = szybkie wł/wył."""
    ...
    def _apply_style(self):
        ...
        if self._active:
            self._lbl.set_markup(
                f'<span foreground="#111111" weight="bold">{self._lbl.get_text()}</span>')
            ...
            button.fm-led {{
                ...
                box-shadow: 0 0 8px {self._color};
                min-height: 26px;
            }}
            """.encode()
            ...
            self._lbl.set_markup(
                f'<span foreground="#888888">{self._lbl.get_text()}</span>')
            provider = Gtk.CssProvider()
            css = b"""
            button.fm-led {
                background: #2a2a2a;
...
class SdrWindow(Adw.ApplicationWindow):
    ...
    def _build_fm_panel(self):
        ...
        legend = Gtk.Label(
            label="EQ niebieski · COMP zielony · ECHO pomarańcz · "
                  "TAPE czerwony · SPAT żółty · PHAN biały",
            xalign=0, wrap=True)
        ...
        # Flat domyślnie
        if "Flat" in self._fm_eq_buttons:
            self._fm_eq_buttons["Flat"].set_active(True)
    ...
    def _on_fm_eq_preset_toggled(self, btn, name):
        ...
        if not self._fx_enabled.get("eq"):
            ...
            if "eq" in self._fx_enable_switches:
