...
FX_ORDER = ("eq", "comp", "echo", "tape", "spatial", "phantom", "fuzz")
...
FX_DEFS = {
    ...
    "tape": {
        "label": "🎛 ATS-1 Tape / Kaseta (typ, Dolby, stereo)",
        "elements": [
...
# Typ kasety: (bias_feel, hi_boost_db, sat_headroom) — mapowane w _map_tape
TAPE_CASSETTE_TYPES = {
    # Wyraźne różnice EQ/saturacji — słyszalne przy przełączaniu
    "Normal (Fe)": {
        "hi": -4.0, "mid": 1.0, "lo": 3.0, "headroom": 0.85,
        "drive_bias": 8.0, "desc": "Type I — cieplejszy bas, stłumione wysokie, więcej saturacji",
    },
    "CrO₂ (Chrome)": {
        "hi": 3.0, "mid": 0.5, "lo": 0.0, "headroom": 1.1,
        "drive_bias": 0.0, "desc": "Type II — jaśniejsze wysokie, czystsza dynamika",
    },
    "Metal": {
        "hi": 6.0, "mid": -0.5, "lo": -1.5, "headroom": 1.35,
        "drive_bias": -6.0, "desc": "Type IV — otwarte wysokie, duży headroom, mniej „mułu”",
    },
}

# Dolby NR — mocniejsza charakterystyka playback (słyszalna zmiana barwy)
TAPE_DOLBY = {
    "Off": {
        "hf": 0.0, "mid": 0.0, "presence": 0.0, "sat_mul": 1.0,
        "desc": "Bez NR — pełne wysokie + naturalny syk/taśma",
    },
    "Dolby B": {
        "hf": -5.0, "mid": -1.0, "presence": 1.5, "sat_mul": 0.85,
        "desc": "Dolby B — wyraźnie cichszy syk w wysokich, lekka obecność",
    },
    "Dolby C": {
        "hf": -9.0, "mid": -2.5, "presence": 2.5, "sat_mul": 0.7,
        "desc": "Dolby C — mocno wygładzone wysokie/środek, „czystszy” odczyt",
    },
}
...
# ==========================================================================
# Panel FM (minimalistyczny, styl Diora / Unitra) — skala + VU + diody
# ==========================================================================
FM_BAND_LO = 88_000_000
FM_BAND_HI = 108_000_000

# Domyślne znaczniki na skali, gdy brak zapisanych stacji (PL-ish)
DEFAULT_FM_PRESETS = [
    ("Radio Maryja", 106_600_000),
    ("RMF FM", 98_800_000),
    ("Radio ZET", 107_500_000),
    ("Trójka", 98_800_000),
    ("Czwórka", 92_000_000),
    ("Radio Kraków", 101_600_000),
]

FX_LED_COLORS = {
    "eq": "#4fc3f7",       # niebieski
    "comp": "#81c784",     # zielony
    "echo": "#ffb74d",     # pomarańczowy
    "tape": "#e57373",     # czerwony
    "spatial": "#fff176",  # żółty
    "phantom": "#e0e0e0",  # biały
    "fuzz": "#ff6ec7",     # różowy / magenta
}
FX_LED_LABELS = {
    "eq": "EQ", "comp": "COMP", "echo": "ECHO",
    "tape": "TAPE", "spatial": "SPAT", "phantom": "PHAN", "fuzz": "FUZZ",
}


class AnalogFmScale(Gtk.DrawingArea):
    """Analogowa skala FM 88–108 MHz ze klikalnymi nazwami stacji."""
    ...
    def __init__(self):
        ...
        self.set_content_height(150)
        self.set_hexpand(True)
        self._freq = 92_000_000
        self._markers = []  # list of (name, hz)
        self._hit_boxes = []  # (x0, y0, x1, y1, hz, name)
        self.on_tune = None
        self.set_draw_func(self._on_draw)
        click = Gtk.GestureClick()
    ...
    def _x_to_hz(self, x, width):
        frac = clamp(x / max(width, 1), 0.0, 1.0)
        return int(FM_BAND_LO + frac * (FM_BAND_HI - FM_BAND_LO))

    def _hz_to_x(self, hz, width):
        frac = (hz - FM_BAND_LO) / (FM_BAND_HI - FM_BAND_LO)
        return clamp(frac, 0.0, 1.0) * width

    def _hit_test(self, x, y):
        for i, (x0, y0, x1, y1, hz, name) in enumerate(self._hit_boxes):
            if x0 <= x <= x1 and y0 <= y <= y1:
                return i, hz, name
        return -1, None, None

    def _on_click(self, gesture, n_press, x, y):
        idx, hz, name = self._hit_test(x, y)
        if hz is None:
            hz = self._x_to_hz(x, self.get_width())
        if self.on_tune:
            self.on_tune(hz)

    def _on_drag(self, gesture, off_x, off_y):
        ok, sx, sy = gesture.get_start_point()
        if not ok:
            return
        # drag po skali (nie po etykiecie) = continuous tune
        idx, _, _ = self._hit_test(sx, sy)
        if idx >= 0:
            return
        hz = self._x_to_hz(sx + off_x, self.get_width())
        self.set_frequency(hz)
        if self.on_tune:
            self.on_tune(hz)

    def _on_motion(self, ctrl, x, y):
        idx, _, _ = self._hit_test(x, y)
    ...
    def _on_draw(self, area, cr, width, height):
        import math as _m
        cr.set_source_rgb(0.12, 0.13, 0.14)
        cr.paint()

        pad_l, pad_r, pad_t = 18, 18, 22
        scale_y = height * 0.42
        usable = width - pad_l - pad_r
        self._hit_boxes = []

        cr.set_source_rgb(0.18, 0.20, 0.18)
        cr.rectangle(pad_l - 6, pad_t - 6, usable + 12, height - pad_t - 4)
        cr.fill()

        # linia skali
        cr.set_source_rgb(0.85, 0.88, 0.75)
        cr.set_line_width(1.5)
        cr.move_to(pad_l, scale_y)
        cr.line_to(pad_l + usable, scale_y)
        cr.stroke()

        for mhz in range(88, 109):
            hz = mhz * 1_000_000
            x = pad_l + self._hz_to_x(hz, usable)
            major = (mhz % 2 == 0)
            tick = 14 if major else 8
            cr.set_source_rgb(0.9, 0.92, 0.8)
            cr.set_line_width(1.2 if major else 0.8)
            cr.move_to(x, scale_y - tick)
            cr.line_to(x, scale_y + 3)
            cr.stroke()
            if major:
                ...
                cr.move_to(x - ext.width / 2, scale_y - tick - 4)
                cr.show_text(label)

        # nazwy stacji — duże, klikalne strefy
        cr.select_font_face("Sans", 0, 1)
        cr.set_font_size(11)
        for i, (name, hz) in enumerate(self._markers):
            if not (FM_BAND_LO <= hz <= FM_BAND_HI):
                continue
            x = pad_l + self._hz_to_x(hz, usable)
            # naprzemiennie góra/dół żeby się nie nakładały
            above = (i % 2 == 0)
            label = name if len(name) <= 14 else name[:13] + "…"
            ext = cr.text_extents(label)
            tw, th = ext.width + 10, 18
            tx = clamp(x - tw / 2, pad_l, pad_l + usable - tw)
            if above:
                ty = scale_y - 36
            else:
                ty = scale_y + 22
            ...
            if hover:
                ...
                cr.set_source_rgba(0.25, 0.55, 0.30, 0.85)
            cr.rectangle(tx, ty, tw, th)
            cr.fill()
            cr.set_source_rgb(0.05, 0.08, 0.05) if hover else cr.set_source_rgb(0.85, 1.0, 0.88)
            if hover:
                cr.set_source_rgb(0.05, 0.1, 0.05)
            else:
                cr.set_source_rgb(0.85, 1.0, 0.88)
            cr.move_to(tx + 5, ty + 13)
            cr.show_text(label)
            # kropka na skali
            cr.set_source_rgb(0.35, 0.95, 0.45)
            cr.arc(x, scale_y + (6 if above else -6), 3.5, 0, 6.2832)
            cr.fill()
            # linia do kropki
            cr.set_source_rgba(0.4, 0.9, 0.5, 0.5)
            cr.set_line_width(1.0)
            cr.move_to(x, ty + (th if above else 0))
            cr.line_to(x, scale_y + (6 if above else -6))
            cr.stroke()
            self._hit_boxes.append((tx, ty, tx + tw, ty + th, hz, name))

        # wskazówka
        x = pad_l + self._hz_to_x(self._freq, usable)
        cr.set_source_rgb(1.0, 0.55, 0.15)
        cr.set_line_width(2.0)
        cr.move_to(x, pad_t - 2)
        cr.line_to(x, scale_y + 18)
        cr.stroke()
        cr.arc(x, pad_t - 2, 4, 0, 6.2832)
        cr.fill()

        cr.set_source_rgb(0.4, 1.0, 0.55)
        cr.select_font_face("Sans", 0, 1)
        cr.set_font_size(15)
        cr.move_to(pad_l, height - 8)
        cr.show_text(f"{self._freq / 1e6:.2f} MHz")



# ==========================================================================
# Glob klimatyczny 3D (GTK4) — kolory = średnia temp. roczna → EQ
# ==========================================================================
GLOBE_LABELS = [
    ("AFRYKA", 7.0, 21.0),
    ("EUROPA", 50.0, 15.0),
    ("AZJA", 50.0, 90.0),
    ("AMERYKA PŁN.", 45.0, -100.0),
    ("AMERYKA PŁD.", -15.0, -60.0),
    ("AUSTRALIA", -25.0, 135.0),
    ("ANTARKTYDA", -80.0, 0.0),
]

# (poly_points, avg_temp_C, name) — kolor z temperatury
GLOBE_CONTINENTS = [
    # Afryka ~25 C
    ([(37, 10), (30, 32), (12, 43), (-11, 41), (-34, 26), (-34, 18), (5, 0),
      (11, -14), (35, -6), (37, 10)], 25.0, "Afryka"),
    # Madagaskar ~24 C
    ([(-12, 49), (-25, 47), (-25, 44), (-12, 49)], 24.0, "Madagaskar"),
    # Europa ~9 C
    ([(36, -9), (43, -9), (44, 8), (38, 24), (41, 29), (46, 38), (55, 38),
      (60, 30), (70, 28), (71, 25), (62, 5), (50, 1), (48, -4), (36, -9)], 9.0, "Europa"),
    # WB ~9 C
    ([(50, -5), (58, -5), (52, 1), (50, -5)], 9.0, "WB"),
    # Skandynawia ~2 C
    ([(55, 12), (69, 16), (70, 28), (60, 30), (59, 18), (55, 12)], 2.0, "Skandynawia"),
    # Azja ~5 C (średnia kontynentalna)
    ([(41, 29), (60, 30), (66, 68), (72, 110), (66, 170), (40, 140), (22, 114),
      (10, 106), (1, 104), (12, 75), (25, 62), (12, 43), (28, 34), (41, 29)], 5.0, "Azja"),
    # Japonia ~12 C
    ([(45, 142), (35, 139), (31, 130), (37, 137), (45, 142)], 12.0, "Japonia"),
    # Ameryka Północna ~5 C
    ([(72, -125), (70, -162), (60, -166), (58, -136), (48, -124), (30, -116),
      (16, -93), (9, -79), (21, -87), (29, -80), (44, -65), (52, -55),
      (60, -64), (72, -125)], 5.0, "Am.Płn."),
    # Grenlandia ~-15 C
    ([(76, -69), (81, -18), (60, -43), (76, -69)], -15.0, "Grenlandia"),
    # Ameryka Południowa ~22 C
    ([(9, -79), (10, -62), (-5, -35), (-22, -41), (-53, -68), (-55, -73),
      (-18, -71), (0, -80), (9, -79)], 22.0, "Am.Płd."),
    # Australia ~22 C
    ([(-12, 130), (-14, 141), (-38, 148), (-35, 117), (-22, 114), (-12, 130)],
     22.0, "Australia"),
    # Antarktyda ~-50 C
    ([(-65, -60), (-67, 0), (-66, 90), (-65, 180), (-70, -90), (-65, -60)],
     -50.0, "Antarktyda"),
]


def _temp_to_rgb(t_c):
    """Mapuje °C na kolor klimatu: lód → zimny → umiarkowany → gorący."""
    # -50..30
    t = max(-50.0, min(30.0, float(t_c)))
    if t < -20:
        # lód / antarktyda: biało-błękit
        u = (t + 50) / 30.0
        return (0.75 + 0.2 * u, 0.88 + 0.1 * u, 0.98)
    if t < 0:
        u = (t + 20) / 20.0
        return (0.35 + 0.2 * u, 0.55 + 0.25 * u, 0.75 + 0.1 * u)
    if t < 12:
        u = t / 12.0
        return (0.25 + 0.15 * u, 0.55 + 0.15 * u, 0.35 - 0.05 * u)
    if t < 20:
        u = (t - 12) / 8.0
        return (0.45 + 0.35 * u, 0.65 - 0.1 * u, 0.25)
    # gorąco
    u = (t - 20) / 10.0
    return (0.85 + 0.1 * u, 0.45 - 0.15 * u, 0.15)


def _geo_to_3d(lat, lon):
    rad_lat = math.radians(lat)
    rad_lon = math.radians(lon)
    x = math.cos(rad_lat) * math.sin(rad_lon)
    y = -math.sin(rad_lat)
    z = math.cos(rad_lat) * math.cos(rad_lon)
    return [x, y, z]


def _rotate_3d(point, rot_x, rot_y):
    x, y, z = point
...
def _climate_eq_bands(lat_focus, temp_c):
    """
    Propozycja EQ 10-band zależna od klimatu / szerokości.
    Zimno (bieguny): cieplejsze EQ (bas+, treble-).
    Gorąco (równik): jaśniejsze (bas-, presence/treble+).
    """
    # lat_focus -90..90, temp -50..30
    cold = clamp((12.0 - temp_c) / 40.0, 0.0, 1.0)   # 1 = zimno
    hot = clamp((temp_c - 8.0) / 25.0, 0.0, 1.0)      # 1 = gorąco
    # 10 pasm jak EQ_PRESETS
    bands = [0.0] * 10
    # bas
    bands[0] = 4.0 * cold - 3.0 * hot
    bands[1] = 3.0 * cold - 2.0 * hot
    bands[2] = 1.5 * cold - 1.0 * hot
    # mid
    bands[3] = -0.5 * cold + 0.5 * hot
    bands[4] = 0.5 * hot
    bands[5] = 1.0 * hot - 0.5 * cold
    # presence / treble
    bands[6] = 1.5 * hot - 1.5 * cold
    bands[7] = 2.5 * hot - 2.5 * cold
    bands[8] = 3.5 * hot - 3.5 * cold
    bands[9] = 4.0 * hot - 4.5 * cold
    return [clamp(b, -12.0, 12.0) for b in bands]


class ClimateGlobe(Gtk.DrawingArea):
    """Globus 3D z zaokrąglonymi krawędziami; obrót → klimat → EQ."""
    ...
    def __init__(self):
        ...
        self._last_x = 0.0
        self._last_y = 0.0
        self.on_climate = None  # callback(lat, temp_c, bands, label)
        self._focus_lat = 0.0
        self._focus_temp = 15.0
        self._focus_label = "umiarkowany"
        self.set_draw_func(self._on_draw)
        ...
        # delikatna inercja / odświeżenie opisu przy starcie
        GLib.idle_add(self._emit_climate)

    def _on_drag_begin(self, gesture, x, y):
        self._dragging = True
        self._last_x = x
        self._last_y = y
        self._rot_x0 = self.rot_x
        self._rot_y0 = self.rot_y

    def _on_drag_update(self, gesture, off_x, off_y):
        self.rot_y = self._rot_y0 + off_x * 0.4
        self.rot_x = clamp(self._rot_x0 - off_y * 0.4, -85.0, 85.0)
        self.queue_draw()
        self._emit_climate()
    ...
    def _emit_climate(self):
        # środek widoku ≈ -rot_x jako szerokość „skupienia”
        lat = clamp(-self.rot_x, -80.0, 80.0)
        # temp. przybliżona z szerokości (nie z konkretnego lądu)
        # biegun -40, równik +28
        temp = 28.0 - abs(lat) * (28.0 + 40.0) / 90.0
        if abs(lat) > 60:
            label = "polarny / zimny"
        elif abs(lat) > 35:
            label = "umiarkowany / chłodny"
        elif abs(lat) > 15:
            label = "subtropikalny"
        else:
            label = "równikowy / gorący"
        bands = _climate_eq_bands(lat, temp)
        self._focus_lat = lat
        self._focus_temp = temp
        self._focus_label = label
        if self.on_climate:
            self.on_climate(lat, temp, bands, label)
        return False
    ...
    def _on_draw(self, area, cr, width, height):
        ...
        # miękki cień pod kulą
        cr.save()
        cr.set_source_rgba(0, 0, 0, 0.35)
        cr.translate(cx, cy + radius * 0.88)
        cr.scale(1.0, 0.22)
        cr.arc(0, 0, radius * 0.95, 0, 2 * math.pi)
        cr.fill()
        cr.restore()

        # ocean — gradient-ish (kilka pierścieni)
        for i in range(8, 0, -1):
            f = i / 8.0
            cr.arc(cx, cy, radius * f, 0, 2 * math.pi)
            cr.set_source_rgb(0.05 + 0.04 * f, 0.12 + 0.10 * f, 0.28 + 0.12 * f)
            cr.fill()

        # obwódka / atmosfera (zaokrąglona krawędź)
        cr.arc(cx, cy, radius, 0, 2 * math.pi)
        cr.set_source_rgba(0.45, 0.7, 1.0, 0.35)
        cr.set_line_width(3.0)
        cr.stroke()
        cr.arc(cx, cy, radius + 2.5, 0, 2 * math.pi)
        cr.set_source_rgba(0.2, 0.35, 0.55, 0.25)
        ...
        for lat in range(-60, 90, 30):
            ...
            for lon in range(-180, 185, 5):
                p = _rotate_3d(_geo_to_3d(lat, lon), self.rot_x, self.rot_y)
                if p[2] > 0:
                    px = cx + p[0] * radius
                    py = cy + p[1] * radius
                    if first:
        ...
        # kontynenty — kolor z temperatury, wypełnienie + obrys
        for poly, temp, _name in GLOBE_CONTINENTS:
            rgb = _temp_to_rgb(temp)
            pts = []
            for lat, lon in poly:
                p = _rotate_3d(_geo_to_3d(lat, lon), self.rot_x, self.rot_y)
                if p[2] > 0.02:
                    pts.append((cx + p[0] * radius, cy + p[1] * radius, p[2]))
            if len(pts) < 3:
                continue
            cr.set_source_rgba(rgb[0], rgb[1], rgb[2], 0.88)
            cr.move_to(pts[0][0], pts[0][1])
            for x, y, _z in pts[1:]:
                cr.line_to(x, y)
            cr.close_path()
            cr.fill_preserve()
            cr.set_source_rgba(rgb[0] * 0.7, rgb[1] * 0.7, rgb[2] * 0.7, 1.0)
        ...
        cr.select_font_face("Sans", 0, 1)
        cr.set_font_size(10)
        for name, lat, lon in GLOBE_LABELS:
            p = _rotate_3d(_geo_to_3d(lat, lon), self.rot_x, self.rot_y)
            if p[2] > 0.15:
                px = cx + p[0] * radius
                py = cy + p[1] * radius
                ...
                cr.rectangle(px - ext.width / 2 - 3, py - ext.height / 2 - 2,
                             ext.width + 6, ext.height + 4)
                cr.fill()
                cr.set_source_rgba(1, 1, 1, 0.9)
                cr.move_to(px - ext.width / 2, py + ext.height / 2 - 1)
                cr.show_text(name)

        # highlight krawędzi (naturalniejsza kula)
        cr.arc(cx - radius * 0.25, cy - radius * 0.3, radius * 0.55, 0, 2 * math.pi)
        cr.set_source_rgba(1, 1, 1, 0.07)
        cr.fill()

        # pasek info klimatu
        cr.set_source_rgb(0.75, 0.8, 0.7)
        cr.set_font_size(10)
        cr.move_to(8, height - 8)
        cr.show_text(f"{self._focus_label}  {self._focus_lat:+.0f}°  ~{self._focus_temp:.0f}°C")



class VuMeterView(Gtk.DrawingArea):
    """Wskaźnik poziomu: tryb 'needle' (VU) lub 'bars' (słupki)."""
    ...
    def __init__(self, label="L"):
        ...
        self._level = 0.0  # 0..1
        self._peak = 0.0
        self._mode = "needle"  # needle | bars
        self.set_draw_func(self._on_draw)

    def set_mode(self, mode):
        self._mode = mode if mode in ("needle", "bars") else "needle"
        self.queue_draw()
    ...
    def _on_draw(self, area, cr, width, height):
        cr.set_source_rgb(0.10, 0.11, 0.12)
        cr.paint()
        cr.set_source_rgb(0.16, 0.17, 0.18)
        ...
        if self._mode == "bars":
            ...
            self._draw_needle(cr, width, height)

        cr.set_source_rgb(0.75, 0.78, 0.7)
        cr.select_font_face("Sans", 0, 0)
        cr.set_font_size(11)
        cr.move_to(8, 14)
        cr.show_text(self._label)

    def _draw_needle(self, cr, width, height):
        cx, cy = width / 2, height - 8
        radius = min(width, height) * 0.72
        # skala
        for i, col in enumerate([(0.3, 0.85, 0.4), (0.3, 0.85, 0.4),
                                  (0.95, 0.85, 0.2), (1.0, 0.45, 0.15),
                                  (1.0, 0.2, 0.15)]):
            a0 = 3.14159 * (0.85 - i * 0.15)
            a1 = 3.14159 * (0.85 - (i + 1) * 0.15)
            cr.set_source_rgb(*col)
            cr.set_line_width(3.0)
            cr.arc(cx, cy, radius, a1, a0)
            cr.stroke()
        # wskazówka
        ang = 3.14159 * (0.85 - self._level * 0.75)
        cr.set_source_rgb(1.0, 0.9, 0.85)
        cr.set_line_width(1.8)
        cr.move_to(cx, cy)
        cr.line_to(cx + radius * 0.9 * __import__("math").cos(ang),
                   cy - radius * 0.9 * __import__("math").sin(ang))
        cr.stroke()
        cr.arc(cx, cy, 3.5, 0, 6.28)
        cr.fill()
    ...
    def _draw_bars(self, cr, width, height):
        ...
        for i in range(n):
            thr = (i + 1) / n
            on = self._level >= thr - 0.01
            if i < 10:
                col = (0.3, 0.9, 0.4) if on else (0.15, 0.25, 0.15)
            elif i < 13:
                col = (1.0, 0.85, 0.2) if on else (0.25, 0.22, 0.08)
            else:
                col = (1.0, 0.25, 0.2) if on else (0.28, 0.1, 0.08)
            h = 8 + i * ((height - 30) / n)
            cr.set_source_rgb(*col)
            cr.rectangle(pad + i * (bw + gap), base - h, bw, h)
            cr.fill()


class LedChip(Gtk.Button):
    """Dioda FX z animacją pulsowania gdy aktywna (unikalna klasa CSS / klucz)."""

    def __init__(self, key, color, label):
        super().__init__()
        self.fx_key = key
        self._color = color
        ...
        self._phase = 0.0
        self._tick_id = 0
        self._css_class = f"fm-led-{key}"
        self._lbl = Gtk.Label(label=label)
        self.set_child(self._lbl)
        self.set_size_request(56, 30)
        self.add_css_class("fm-led")
    ...
    def set_led_active(self, active):
        active = bool(active)
        if active == self._active and self._tick_id:
            return
        was = self._active
        self._active = active
        if active and not self._tick_id:
            self._tick_id = GLib.timeout_add(50, self._animate)
        elif not active:
            if self._tick_id:
                try:
    ...
    def _animate(self):
        ...
        bright = 0.5 + 0.5 * (0.5 + 0.5 * math.sin(self._phase))
        self._apply_style(bright)
        return True

    def _apply_style(self, bright=1.0):
        ...
        cls = self._css_class
        if self._active:
            r = min(255, int(r * (0.55 + 0.45 * bright)))
            g = min(255, int(g * (0.55 + 0.45 * bright)))
            b = min(255, int(b * (0.55 + 0.45 * bright)))
            bg = f"#{r:02x}{g:02x}{b:02x}"
            fg = "#111111"
            glow = self._color
            weight = "bold"
            shadow = f"0 0 10px {glow}"
        else:
            bg = "#2a2a2a"
            fg = "#777777"
            weight = "normal"
        ...
        self._lbl.set_markup(
            f'<span foreground="{fg}" weight="{weight}">{self._label_txt}</span>')
...
class SdrWindow(Adw.ApplicationWindow):
    def __init__(self, app, host, port, frequency, sample_rate):
        ...
        self._fx_ext_state = {
            "tape": {
                "drive": 50.0, "warmth": 30.0, "comp": 20.0,
                "cassette": "Normal (Fe)", "dolby": "Off",
                "stereo_mode": "Stereo", "prologic": False, "mpx": False,
            },
            "spatial": {"width": 0.0, "haas": 0.0, "sat": 10.0},
            "phantom": {"mode": "off", "width": 5.0, "delay": 0.0, "blend": 0.0},
            "fuzz": {"amount": 45.0, "tone": 40.0, "level": 55.0},
        }
    ...
    def _find_peak(self, mag_c, center, bin_hz):
        ...
        # width_bins WLACZNIE (right - left + 1)
        width_bins = right - left + 1
        if width_bins < MIN_PEAK_WIDTH_BINS:
            return None

        # Lagodniejszy margines do pomiaru pasma + pad IF
    ...
    def _on_spectrum_frame(self, disp_i, disp_q, peak, disp_c=None):
        ...
        self._update_fm_vu(disp_i, disp_q)
        if hasattr(self, "fm_scale"):
            self.fm_scale.set_frequency(center)
        return False
    ...
    def _map_tape(s):
        ...
        cass = TAPE_CASSETTE_TYPES.get(
            s.get("cassette", "Normal (Fe)"), TAPE_CASSETTE_TYPES["Normal (Fe)"])
        dolby = TAPE_DOLBY.get(s.get("dolby", "Off"), TAPE_DOLBY["Off"])
        stereo_mode = s.get("stereo_mode", "Stereo")
        prologic = bool(s.get("prologic", False))
        mpx = bool(s.get("mpx", False))

        # Drive efektywny: typ kasety przesuwa "nasycenie" (Fe szybciej się klei)
        eff_drive = clamp(drive + cass.get("drive_bias", 0.0), 0.0, 100.0)
        headroom = max(cass.get("headroom", 1.0), 0.5)
        thr = 0.92 - (eff_drive / 100.0 * 0.85) / headroom
        thr = clamp(thr, 0.08, 0.95)
        vol = 0.95 + (eff_drive / 100.0 * 0.9)
        ratio = (1.2 + (comp / 100.0 * 10.0)) * dolby.get("sat_mul", 1.0)
        ratio = clamp(ratio, 1.0, 12.0)

        # Barwa: warmth + kaseta + Dolby — większe skoki dB = słyszalna zmiana
        band0 = warmth / 100.0 * 5.0 + cass["lo"]
        band1 = cass.get("mid", 0.0) + dolby.get("mid", 0.0) + dolby.get("presence", 0.0)
        band2 = -(warmth / 100.0 * 4.0) + cass["hi"] + dolby.get("hf", 0.0)
        if mpx:
            # mocne sciecie skrajnych wysokich (pilot FM / MPX)
            band2 -= 7.0
            band1 -= 1.5

        # Stereo / Mono / Pro Logic (szerokosc + Haas)
        if stereo_mode == "Mono":
            stereo_w = 0.0
            echo_delay = 1
            echo_int = 0.0
        elif prologic:
            # wyraznie szersza scena + krotki Haas (~8 ms)
            stereo_w = 0.75
            echo_delay = 8_000_000  # 8 ms
            echo_int = 0.28
        else:
            stereo_w = 1.0
            echo_delay = 1
        ...
        return {
            ("sat", "threshold"): thr,
            ("gain", "volume"): vol,
            ("tone", "band0"): clamp(band0, -24.0, 12.0),
            ("tone", "band1"): clamp(band1, -24.0, 12.0),
            ("tone", "band2"): clamp(band2, -24.0, 12.0),
            ("sat", "ratio"): ratio,
            ("sw", "stereo"): stereo_w,
            ("echo", "delay"): echo_delay,
            ("echo", "intensity"): echo_int,
            ("echo", "feedback"): 0.0,
        }
    ...
    @staticmethod
    def _map_fuzz(s):
        amount = s.get("amount", 45.0)
        tone = s.get("tone", 40.0)
        level = s.get("level", 55.0)
        # hard-knee + niski threshold = agresywny fuzz
        thr = 0.85 - (amount / 100.0 * 0.75)
        ratio = 2.0 + (amount / 100.0 * 14.0)
        vol = 0.7 + (level / 100.0 * 0.9)
        # tone: wiecej = jasniejszy fuzz
        band0 = (50.0 - tone) / 50.0 * 3.0
        band2 = (tone - 50.0) / 50.0 * 6.0
        return {
            ("sat", "threshold"): clamp(thr, 0.05, 0.9),
            ("sat", "ratio"): clamp(ratio, 1.5, 16.0),
            ("gain", "volume"): clamp(vol, 0.3, 2.0),
            ("tone", "band0"): clamp(band0, -12.0, 12.0),
            ("tone", "band1"): 0.0,
            ("tone", "band2"): clamp(band2, -12.0, 12.0),
        }
    ...
    def _apply_fx_live(self, key):
        ...
        if key == "eq":
            ...
            mapper = {"tape": self._map_tape, "spatial": self._map_spatial,
                      "phantom": self._map_phantom, "fuzz": self._map_fuzz}[key]
    ...
    def _build_ui(self):
        ...
        # Przełącznik trybu: Panel FM (start) / Zaawansowane
        mode_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
        self._btn_mode_fm = Gtk.ToggleButton(label="Panel FM", active=True)
        self._btn_mode_adv = Gtk.ToggleButton(label="Zaawansowane")
        self._btn_mode_fm.add_css_class("fm-mode-btn")
        self._btn_mode_adv.add_css_class("fm-mode-btn")
        self._btn_mode_adv.set_group(self._btn_mode_fm)
        self._btn_mode_fm.connect("toggled", self._on_mode_toggled)
        mode_box.append(self._btn_mode_fm)
        mode_box.append(self._btn_mode_adv)
        header.pack_end(mode_box)

        self._mode_stack = Gtk.Stack(hexpand=True, vexpand=True,
                                      transition_type=Gtk.StackTransitionType.CROSSFADE)
        toolbar.set_content(self._mode_stack)
        self.set_content(toolbar)

        # --- tryb minimalistyczny ---
        self._mode_stack.add_named(self._build_fm_panel(), "fm")

        # --- tryb zaawansowany (sidebar + stack) ---
        adv = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
        self.stack = Gtk.Stack(hexpand=True, vexpand=True,
                                transition_type=Gtk.StackTransitionType.CROSSFADE)
        sidebar = Gtk.StackSidebar(stack=self.stack, vexpand=True)
        sidebar.set_size_request(200, -1)
        adv.append(sidebar)
        adv.append(Gtk.Separator(orientation=Gtk.Orientation.VERTICAL))
        adv.append(self.stack)
        self._mode_stack.add_named(adv, "adv")
        self._mode_stack.set_visible_child_name("fm")
        ...
        self._refresh_fm_panel_markers()
        self._sync_fm_panel_leds()
    ...
    def _apply_css(self):
        ...
        /* Panel FM - styl segmentowy Diora / Unitra */
        .fm-panel {
        ...
        .fm-stereo-on { color: #5CFF8A; font-weight: bold; }
        .fm-stereo-off { color: #555555; }
        .fm-seg-label {
            font-size: 10px;
            color: #9aa090;
            letter-spacing: 1px;
        ...
        button.fm-eq-btn:checked, button.fm-eq-btn.active {
            background: #4fc3f7;
            color: #111;
            border-color: #81d4fa;
        }
    ...
    def _on_mode_toggled(self, btn):
        if self._btn_mode_fm.get_active():
            self._mode_stack.set_visible_child_name("fm")
            self._refresh_fm_panel_markers()
            self._sync_fm_panel_leds()
        else:
            self._mode_stack.set_visible_child_name("adv")

    def _build_fm_panel(self):
        """Minimalistyczny panel FM — fonie audio, bez strojenia technicznego."""
        page = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10,
                        margin_top=12, margin_bottom=12,
                        margin_start=16, margin_end=16)
        page.add_css_class("fm-panel")

        # nagłówek segmentowy
        head = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=12)
        title = Gtk.Label(label="FM  ·  UNITRA / DIORA  ·  AUDIO", xalign=0)
        title.add_css_class("fm-title")
        head.append(title)
        head.append(Gtk.Box(hexpand=True))  # spacer
        self._fm_stereo_lbl = Gtk.Label(label="STEREO")
        self._fm_stereo_lbl.add_css_class(
            "fm-stereo-on" if self._current_stereo else "fm-stereo-off")
        head.append(self._fm_stereo_lbl)
        self._fm_denoise_led = Gtk.Label(label="  NR  ")
        self._fm_denoise_led.add_css_class("fm-seg-label")
        head.append(self._fm_denoise_led)
        page.append(head)

        # skala analogowa 88–108
        scale_frame = Gtk.Frame(label="SKALA FM  88 — 108 MHz")
        scale_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4,
                             margin_top=6, margin_bottom=6,
                             margin_start=6, margin_end=6)
        scale_frame.set_child(scale_box)
        self.fm_scale = AnalogFmScale()
        self.fm_scale.set_frequency(self._init_freq)
        self.fm_scale.on_tune = self._on_fm_panel_tune
        scale_box.append(self.fm_scale)
        hint = Gtk.Label(
            label="Klik / przeciągnij po skali = strojenie  ·  "
                  "zielone kropki = presety stacji",
            xalign=0, wrap=True)
        hint.add_css_class("fm-seg-label")
        scale_box.append(hint)
        page.append(scale_frame)

        # rząd: VU + glob klimatyczny (EQ północ/południe)
        mid_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=12)

        vu_frame = Gtk.Frame(label="POZIOM")
        vu_outer = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6,
                            margin_top=6, margin_bottom=6,
                            margin_start=6, margin_end=6)
        vu_frame.set_child(vu_outer)
        vu_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=12)
        self.vu_left = VuMeterView("L")
        self.vu_right = VuMeterView("R")
        vu_row.append(self.vu_left)
        vu_row.append(self.vu_right)
        vu_outer.append(vu_row)
        mode_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        mode_row.append(Gtk.Label(label="Wskaźnik:", xalign=0))
        self._vu_mode_needle = Gtk.ToggleButton(label="VU (wskazówka)", active=True)
        self._vu_mode_bars = Gtk.ToggleButton(label="Słupki")
        self._vu_mode_bars.set_group(self._vu_mode_needle)
        self._vu_mode_needle.connect("toggled", self._on_vu_mode_toggled)
        mode_row.append(self._vu_mode_needle)
        mode_row.append(self._vu_mode_bars)
        vu_outer.append(mode_row)
        mid_row.append(vu_frame)

        globe_frame = Gtk.Frame(label="KLIMAT → EQ  (obróć glob)")
        globe_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4,
                             margin_top=4, margin_bottom=4,
                             margin_start=4, margin_end=4)
        globe_frame.set_child(globe_box)
        self.climate_globe = ClimateGlobe()
        self.climate_globe.on_climate = self._on_globe_climate
        globe_box.append(self.climate_globe)
        self._globe_eq_lbl = Gtk.Label(
            label="Umiarkowany — EQ neutralny", xalign=0, wrap=True)
        self._globe_eq_lbl.add_css_class("fm-seg-label")
        globe_box.append(self._globe_eq_lbl)
        self._globe_eq_auto = Gtk.CheckButton(label="Auto-EQ z klimatu globu")
        self._globe_eq_auto.set_active(True)
        globe_box.append(self._globe_eq_auto)
        mid_row.append(globe_frame)

        page.append(mid_row)

        # diody FX — szybkie przełączanie
        fx_frame = Gtk.Frame(label="EFEKTY  (szybkie wł/wył)")
        fx_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6,
                          margin_top=6, margin_bottom=6,
                          margin_start=6, margin_end=6)
        fx_frame.set_child(fx_box)
        led_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        self._fm_fx_leds = {}
        for key in FX_ORDER:
            led = LedChip(key, FX_LED_COLORS[key], FX_LED_LABELS[key])
            led.connect("clicked", self._on_fm_fx_led_clicked)
            led_row.append(led)
            self._fm_fx_leds[key] = led
        fx_box.append(led_row)
        legend = Gtk.Label(
            label="EQ niebieski · COMP zielony · ECHO pomarańcz · "
                  "TAPE czerwony · SPAT żółty · PHAN biały · FUZZ różowy",
            xalign=0, wrap=True)
        legend.add_css_class("fm-seg-label")
        fx_box.append(legend)
        page.append(fx_frame)

        # presety EQ
        eq_frame = Gtk.Frame(label="PRESET EQ")
        eq_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6,
                          margin_top=6, margin_bottom=6,
                          margin_start=6, margin_end=6)
        eq_frame.set_child(eq_box)
        eq_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
        self._fm_eq_buttons = {}
        for name in EQ_PRESETS:
            btn = Gtk.ToggleButton(label=name)
            btn.add_css_class("fm-eq-btn")
            btn.connect("toggled", self._on_fm_eq_preset_toggled, name)
            eq_row.append(btn)
            self._fm_eq_buttons[name] = btn
        # Flat jako domyslny wyglad (bez set_active — unikamy rebuild
        # zanim powstaną widgety zaawansowane / freq_slider)
        eq_box.append(eq_row)
        page.append(eq_frame)

        # presety stacji (lista skrótowa)
        st_frame = Gtk.Frame(label="STACJE / PRESETY NA SKALI")
        st_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4,
                          margin_top=6, margin_bottom=6,
                          margin_start=6, margin_end=6)
        st_frame.set_child(st_box)
        self._fm_stations_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        self._fm_stations_box.set_halign(Gtk.Align.START)
        st_scroll = Gtk.ScrolledWindow(hexpand=True)
        st_scroll.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.NEVER)
        st_scroll.set_child(self._fm_stations_box)
        st_box.append(st_scroll)
        page.append(st_frame)

        return page

    def _refresh_fm_panel_markers(self):
        if not hasattr(self, "fm_scale"):
            return
        stations = []
        try:
            stations = list(self.stations.load_all())
        except Exception:
            stations = []
        markers = [(s["name"], int(s["frequency"])) for s in stations
                   if FM_BAND_LO <= int(s["frequency"]) <= FM_BAND_HI]
        if not markers:
            markers = list(DEFAULT_FM_PRESETS)
        self.fm_scale.set_markers(markers)
        self.fm_scale.set_frequency(self._current_freq())

        # przyciski stacji
        if hasattr(self, "_fm_stations_box"):
            child = self._fm_stations_box.get_first_child()
            while child is not None:
                nxt = child.get_next_sibling()
                self._fm_stations_box.remove(child)
                child = nxt
            for name, hz in markers:
                b = Gtk.Button(label=f"{name}\n{hz/1e6:.1f}")
                b.add_css_class("fm-eq-btn")
                b.connect("clicked", lambda _b, f=hz: self._on_fm_panel_tune(f))
                self._fm_stations_box.append(b)

    def _on_fm_panel_tune(self, freq_hz):
        freq_hz = int(round(clamp(freq_hz, FM_BAND_LO, FM_BAND_HI)))
        try:
            self.sdrsrc.set_property("frequency", freq_hz)
            self.sdrdemod.set_property("freq-offset", 0.0)
        except Exception as e:
            print(f"[fm panel] tune: {e}", file=sys.stderr)
        if hasattr(self, "freq_slider"):
            self.freq_slider.set_value_quiet(freq_hz)
        if hasattr(self, "freq_offset_slider"):
            self.freq_offset_slider.set_value_quiet(0.0)
        if hasattr(self, "freq_entry"):
            self.freq_entry.set_text(fmt_freq(freq_hz))
        if hasattr(self, "fm_scale"):
            self.fm_scale.set_frequency(freq_hz)
        self.status_lbl.set_label(f"FM: {fmt_freq(freq_hz)}")

    def _on_vu_mode_toggled(self, btn):
        mode = "needle" if self._vu_mode_needle.get_active() else "bars"
        if hasattr(self, "vu_left"):
            self.vu_left.set_mode(mode)
            self.vu_right.set_mode(mode)

    def _on_fm_fx_led_clicked(self, btn):
        key = btn.fx_key
        new_state = not self._fx_enabled.get(key, False)
        self._fx_enabled[key] = new_state
        # zsynchronizuj switch w zaawansowanym UI jeśli istnieje
        if key in self._fx_enable_switches:
            self._fx_enable_switches[key].set_active(new_state)
        else:
    ...
    def _sync_fm_panel_leds(self):
        if not hasattr(self, "_fm_fx_leds"):
            return
        for key, led in self._fm_fx_leds.items():
            led.set_led_active(self._fx_enabled.get(key, False))
        if hasattr(self, "_fm_stereo_lbl"):
            self._fm_stereo_lbl.set_css_classes(
                ["fm-stereo-on"] if self._current_stereo else ["fm-stereo-off"])
        if hasattr(self, "_fm_denoise_led"):
            try:
                on = bool(self.sdrdenoise.get_property("enabled"))
            except Exception:
                on = False
            self._fm_denoise_led.set_markup(
                f'<span foreground="{"#5CFF8A" if on else "#555"}" weight="bold">  NR  </span>')

    def _on_fm_eq_preset_toggled(self, btn, name):
        if not btn.get_active():
            return
        # radio-behaviour
        for n, b in self._fm_eq_buttons.items():
            if n != name and b.get_active():
                b.set_active(False)
        bands = EQ_PRESETS.get(name)
        if not bands:
            return
        self._eq_state["base_bands"] = [float(v) for v in bands]
        if hasattr(self, "_eq_band_sliders"):
            for i, sl in enumerate(self._eq_band_sliders):
                if i < len(bands):
                    sl.set_value_quiet(bands[i])
        if hasattr(self, "eq_preset_combo"):
            # ustaw combo jeśli ma taką nazwę
            model_names = list(EQ_PRESETS.keys())
            if name in model_names:
                self.eq_preset_combo.set_active(model_names.index(name))
        # włącz EQ jeśli wyłączony — żeby preset był słyszalny
        if not self._fx_enabled.get("eq"):
            self._fx_enabled["eq"] = True
            if "eq" in getattr(self, "_fx_enable_switches", {}):
                self._fx_enable_switches["eq"].set_active(True)
            elif hasattr(self, "freq_slider"):
                self._full_rebuild()
            # else: UI jeszcze w budowie — EQ włączy się przy pierwszym restarcie
        elif self._fx_enabled.get("eq"):
            self._apply_fx_live("eq")
        self._sync_fm_panel_leds()

    def _on_globe_climate(self, lat, temp_c, bands, label):
        """Obrót globu → propozycja EQ (zimno=ciepłe EQ, gorąco=jasne)."""
        if hasattr(self, "_globe_eq_lbl"):
            side = "N" if lat >= 0 else "S"
            self._globe_eq_lbl.set_label(
                f"{label}  |  {abs(lat):.0f}°{side}  ~{temp_c:.0f}°C  |  "
                f"bas {bands[0]:+.1f} … treble {bands[9]:+.1f} dB")
        if not hasattr(self, "_globe_eq_auto") or not self._globe_eq_auto.get_active():
            return
        self._eq_state["base_bands"] = [float(v) for v in bands]
        if hasattr(self, "_eq_band_sliders"):
            for i, sl in enumerate(self._eq_band_sliders):
                if i < len(bands):
                    sl.set_value_quiet(bands[i])
        # Tylko live — bez rebuild przy każdym ruchu myszy.
        # Jeśli EQ wyłączone: zapamiętaj pasma; użytkownik włącza diodą EQ.
        if self._fx_enabled.get("eq"):
            self._apply_fx_live("eq")
        self._sync_fm_panel_leds()

    def _update_fm_vu(self, disp_i, disp_q):
        if not hasattr(self, "vu_left"):
            return
        # poziom z energii widma (przybliżenie VU)
        def _lev(arr):
            if arr is None or len(arr) == 0:
                return 0.0
            # normalizacja względem typowego zakresu dB (~ -80..0)
            peak = float(np.max(arr))
            return clamp((peak + 70.0) / 70.0, 0.0, 1.0)
        self.vu_left.set_level(_lev(disp_i))
        self.vu_right.set_level(_lev(disp_q))
    ...
    def _build_fx_creative_group(self):
        ...
        # -- Tape (1-5: typ kasety, Dolby, stereo, Pro Logic, MPX) --
        frame_t, box_t = self._group(FX_DEFS["tape"]["label"])
        self._fx_enable_row("tape", box_t)
        s = self._fx_ext_state["tape"]

        # 1) Typ kasety Fe / CrO2 / Metal
        row_c = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        row_c.append(Gtk.Label(label="1. Typ kasety:", xalign=0))
        cass_combo = Gtk.ComboBoxText()
        for name in TAPE_CASSETTE_TYPES:
            cass_combo.append_text(name)
        keys_c = list(TAPE_CASSETTE_TYPES.keys())
        cass_combo.set_active(keys_c.index(s.get("cassette", keys_c[0]))
                              if s.get("cassette") in keys_c else 0)
        cass_combo.connect("changed", self._on_tape_cassette_changed)
        row_c.append(cass_combo)
        box_t.append(row_c)
        self._fx_sensitive_widgets["tape"].append(cass_combo)
        self._tape_cassette_combo = cass_combo
        self._tape_cassette_desc = Gtk.Label(
            label=TAPE_CASSETTE_TYPES[keys_c[cass_combo.get_active()]]["desc"],
            xalign=0, wrap=True)
        self._tape_cassette_desc.add_css_class("sdr-dim")
        box_t.append(self._tape_cassette_desc)

        # 2) Dolby NR
        row_d = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        row_d.append(Gtk.Label(label="2. Dolby NR:", xalign=0))
        dolby_combo = Gtk.ComboBoxText()
        for name in TAPE_DOLBY:
            dolby_combo.append_text(name)
        keys_d = list(TAPE_DOLBY.keys())
        dolby_combo.set_active(keys_d.index(s.get("dolby", keys_d[0]))
                               if s.get("dolby") in keys_d else 0)
        dolby_combo.connect("changed", self._on_tape_dolby_changed)
        row_d.append(dolby_combo)
        box_t.append(row_d)
        self._fx_sensitive_widgets["tape"].append(dolby_combo)
        self._tape_dolby_combo = dolby_combo
        self._tape_dolby_desc = Gtk.Label(
            label=TAPE_DOLBY[keys_d[dolby_combo.get_active()]]["desc"],
            xalign=0, wrap=True)
        self._tape_dolby_desc.add_css_class("sdr-dim")
        box_t.append(self._tape_dolby_desc)

        # 3) Stereo / Mono
        row_s = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        row_s.append(Gtk.Label(label="3. Stereo kasety:", xalign=0, hexpand=True))
        stereo_combo = Gtk.ComboBoxText()
        for name in ("Stereo", "Mono"):
            stereo_combo.append_text(name)
        stereo_combo.set_active(0 if s.get("stereo_mode", "Stereo") == "Stereo" else 1)
        stereo_combo.connect("changed", self._on_tape_stereo_changed)
        row_s.append(stereo_combo)
        box_t.append(row_s)
        self._fx_sensitive_widgets["tape"].append(stereo_combo)
        self._tape_stereo_combo = stereo_combo

        # 4) Pro Logic (lekkie poszerzenie sceny)
        row_p = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        pl_lbl = Gtk.Label(label="4. Pro Logic (szerokość sceny)", xalign=0, hexpand=True)
        pl_sw = Gtk.Switch(active=bool(s.get("prologic", False)), valign=Gtk.Align.CENTER)
        pl_sw.connect("state-set", self._on_tape_prologic_toggled)
        row_p.append(pl_lbl)
        row_p.append(pl_sw)
        box_t.append(row_p)
        self._fx_sensitive_widgets["tape"].append(pl_sw)
        self._tape_prologic_switch = pl_sw

        # 5) MPX filter (nagrywanie z FM)
        row_m = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        mpx_lbl = Gtk.Label(label="5. Filtr MPX (FM 19 kHz)", xalign=0, hexpand=True)
        mpx_sw = Gtk.Switch(active=bool(s.get("mpx", False)), valign=Gtk.Align.CENTER)
        mpx_sw.connect("state-set", self._on_tape_mpx_toggled)
        row_m.append(mpx_lbl)
        row_m.append(mpx_sw)
        box_t.append(row_m)
        self._fx_sensitive_widgets["tape"].append(mpx_sw)
        self._tape_mpx_switch = mpx_sw

        box_t.append(Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL))
        ...
        # -- Fuzz --
        frame_f, box_f = self._group(FX_DEFS["fuzz"]["label"])
        self._fx_enable_row("fuzz", box_f)
        fs = self._fx_ext_state["fuzz"]
        self._fuzz_sliders = {}
        for key, label, lo, hi, val in (
                ("amount", "Amount (fuzz)", 0.0, 100.0, fs["amount"]),
                ("tone", "Tone", 0.0, 100.0, fs["tone"]),
                ("level", "Level", 0.0, 100.0, fs["level"])):
            slider = SimpleSlider(
                label, lo, hi, 1.0, val, digits=0,
                on_change=self._make_ext_fx_handler("fuzz", key))
            slider.set_sensitive(False)
            box_f.append(slider)
            self._fx_sensitive_widgets["fuzz"].append(slider)
            self._fuzz_sliders[key] = slider
        page.append(frame_f)
    ...
    def _on_tape_cassette_changed(self, combo):
        name = combo.get_active_text()
        if not name:
            return
        self._fx_ext_state["tape"]["cassette"] = name
        info = TAPE_CASSETTE_TYPES.get(name, {})
        if hasattr(self, "_tape_cassette_desc"):
            self._tape_cassette_desc.set_label(info.get("desc", ""))
        if self._fx_enabled.get("tape"):
            self._apply_fx_live("tape")

    def _on_tape_dolby_changed(self, combo):
        name = combo.get_active_text()
        if not name:
            return
        self._fx_ext_state["tape"]["dolby"] = name
        info = TAPE_DOLBY.get(name, {})
        if hasattr(self, "_tape_dolby_desc"):
            self._tape_dolby_desc.set_label(info.get("desc", ""))
        if self._fx_enabled.get("tape"):
            self._apply_fx_live("tape")

    def _on_tape_stereo_changed(self, combo):
        name = combo.get_active_text() or "Stereo"
        self._fx_ext_state["tape"]["stereo_mode"] = name
        if self._fx_enabled.get("tape"):
            self._apply_fx_live("tape")

    def _on_tape_prologic_toggled(self, switch, state):
        self._fx_ext_state["tape"]["prologic"] = bool(state)
        if self._fx_enabled.get("tape"):
            self._apply_fx_live("tape")
        return False

    def _on_tape_mpx_toggled(self, switch, state):
        self._fx_ext_state["tape"]["mpx"] = bool(state)
        if self._fx_enabled.get("tape"):
            self._apply_fx_live("tape")
        return False
    ...
    def _apply_state(self, s):
        ...
        # tape - odtworz combosy/switchy z fx_ext_state
        ts = self._fx_ext_state.get("tape", {})
        if hasattr(self, "_tape_cassette_combo"):
            keys_c = list(TAPE_CASSETTE_TYPES.keys())
            cname = ts.get("cassette", keys_c[0])
            self._tape_cassette_combo.set_active(
                keys_c.index(cname) if cname in keys_c else 0)
            self._tape_cassette_combo.set_sensitive(self._fx_enabled.get("tape", False))
        if hasattr(self, "_tape_dolby_combo"):
            keys_d = list(TAPE_DOLBY.keys())
            dname = ts.get("dolby", keys_d[0])
            self._tape_dolby_combo.set_active(
                keys_d.index(dname) if dname in keys_d else 0)
            self._tape_dolby_combo.set_sensitive(self._fx_enabled.get("tape", False))
        if hasattr(self, "_tape_stereo_combo"):
            self._tape_stereo_combo.set_active(
                0 if ts.get("stereo_mode", "Stereo") == "Stereo" else 1)
            self._tape_stereo_combo.set_sensitive(self._fx_enabled.get("tape", False))
        if hasattr(self, "_tape_prologic_switch"):
            self._tape_prologic_switch.set_active(bool(ts.get("prologic", False)))
            self._tape_prologic_switch.set_sensitive(self._fx_enabled.get("tape", False))
        if hasattr(self, "_tape_mpx_switch"):
            self._tape_mpx_switch.set_active(bool(ts.get("mpx", False)))
            self._tape_mpx_switch.set_sensitive(self._fx_enabled.get("tape", False))

        # spatial/phantom - odtworz suwaki z fx_ext_state (best-effort,
    ...
    def _rebind_prop_widgets(self):
        # Panel FM moze wywolac rebuild zanim powstana widgety zaawansowane
        if not hasattr(self, "freq_slider"):
            for key in FX_ORDER:
                if self._fx_enabled.get(key):
                    self._apply_fx_live(key)
            return
